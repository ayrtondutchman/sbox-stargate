...
public class PuddleJumperCamera : CameraMode
{
	...
	{
		var car = Entity as PuddleJumper;
		if ( !car.IsValid() ) return;

		var body = car.PhysicsBody;
		if ( !body.IsValid() ) return;

		var speed = car.yawSpeed;
		...
		carPitch = carPitch.LerpTo( car.Driver.IsValid() ? carRot.Pitch().Clamp( MinCarPitch, MaxCarPitch ) * (speed < 0.0f ? -1.0f : 1.0f) : 0.0f, Time.Delta * CarPitchSmoothingSpeed );
	...
	private void DoThirdPerson( PuddleJumper car, PhysicsBody body )
	{
	...
	public override void BuildInput()
	{
		...
		var pawn = (Local.Pawn as PuddleJumper).Driver;
		if ( pawn == null ) return;
